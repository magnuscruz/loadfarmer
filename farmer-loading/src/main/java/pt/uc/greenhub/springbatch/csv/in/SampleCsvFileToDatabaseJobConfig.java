 ...
 */
//@Configuration
public class SampleCsvFileToDatabaseJobConfig {

	private static final String PROPERTY_CSV_SOURCE_FILE_PATH = "csv.to.database.job.source.file.path.sample";
	...
	private LineTokenizer createStudentLineTokenizer() {
		...
		studentLineTokenizer.setNames(new String[] { "id", "deviceId", "timestamp", "appVersion", "databaseVersion",
				"batteryStage", "batteryLevel", "memoryWired", "memoryActive", "memoryInactive", "memoryFree",
				"memoryUser", "triggered", "networkStatus", "screenBrightness", "screenOn", "timezone", "countryCode",
				"created", "updated" });
	...
	private FieldSetMapper<SampleDTO> createStudentInformationMapper() {
		BeanWrapperFieldSetMapper<SampleDTO> studentInformationMapper = new BeanWrapperFieldSetMapperCustom<>(DATE_FORMAT);
	...
	@Bean
	Step sampleCsvFileToDatabaseStep(ItemReader<SampleDTO> csvFileItemReader,
			ItemProcessor<SampleDTO, SampleDTO> csvFileItemProcessor, ItemWriter<SampleDTO> csvFileDatabaseItemWriter,
			StepBuilderFactory stepBuilderFactory) {
		return stepBuilderFactory.get("sampleCsvFileToDatabaseStep").<SampleDTO, SampleDTO>chunk(1).reader(csvFileItemReader)
				.faultTolerant().skipPolicy(fileVerificationSkipper()).processor(csvFileItemProcessor)
				.writer(csvFileDatabaseItemWriter).faultTolerant().skipPolicy(fileVerificationSkipper()).build();
	}

	@Bean
	Job sampleCsvFileToDatabaseJob(JobBuilderFactory jobBuilderFactory,
			@Qualifier("sampleCsvFileToDatabaseStep") Step csvStudentStep) {
		return jobBuilderFactory.get("sampleCsvFileToDatabaseJob").incrementer(new RunIdIncrementer()).flow(csvStudentStep)
				.end().build();
	}
}
